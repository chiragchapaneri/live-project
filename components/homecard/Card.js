...
export const Card = ({ data, index, eventForMe, bookMarkedEvents }) => {
  ...
  return (
    ...
    >
      {token && (
        <div className="absolute z-[1] right-10 top-10 cursor-pointer bg-[#7F7F7F] rounded-5 p-3">
          {isLoding && id === data.id ? (
            <Oval
              color="#FFFFFF"
              height={14}
      ...
      <div className="relative h-181 w-full rounded-t-5 overflow-hidden">
        <Image
          ...
          objectFit="cover"
        />
        <button className="absolute text-10 leading-5 font-normal px-7 rounded-5 top-127 sm:top-145 md:top-145 lg:top-145 right-15 lg:text-xs sm:text-10 sm:leading-5 sm:font-normal lg:h-22 lg:px-7 sm:px-7 bg-light-blue ">
          {data?.type?.name}
        </button>
      </div>
      <div className=" px-22 pt-18">
        <div className="text-base lg:text-base sm:text-14 sm:leading-6 sm:font-medium truncate">
          {data?.name}
        </div>
