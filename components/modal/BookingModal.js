...
const BookingModal = (props) => {
  ...
  const onChange = (e) => {
    if (ticketBook?.length <= 0 || !ticketBook) {
      if (e?.value >= 1) {
        const ticketData = [];
      ...
      const find = ticketBook?.filter((data, index) => {
      ...
      if (find.length >= 1) {
        setShowMessage(false);
        if (e?.value > 0) {
          const updateData = [...ticketBook];
          const index = updateData.findIndex((data) => {
            return data.id == e.id;
          });

          updateData[index].count = e.value;
          updateData[index].totalPrice = e.value * e.price;

          const sum = updateData.reduce(
            (previous, current) => previous + current.totalPrice,
            0
          );

          setTicketBook(updateData);
          SetTotalPrice(sum);
        } else {
          const updateData = ticketBook?.filter((data, index) => {
            return data.id !== e.id && data;
          });
          const sum = updateData.reduce(
            (previous, current) => previous + current.totalPrice,
  ...
  return (
    <>
      ...
      <Transition appear show={showModel} as={Fragment}>
        <Dialog as="div" className="relative z-99" onClose={() => openModal()}>
          <Transition.Child
            as={Fragment}
